...
import pprint
import ntpath
from auxiliar.external import http_server
import numpy as np
# import vt_graph_api as vtg
...
class Anything(metaclass=ABCMeta):
    ...
    #       CONSTRUCTOR
    # ---------------------------------
    def __init__(self, id, others=None, notlistable=[OBJECTS_TO_PROCESS], statistics=True, plotHistory=False):
        if id is None:
    ...
    def toJSON(self):
        ret = {}
        ret.update({'id':self.getId(), 'type':self.__class__, 'backpack':self.getBackpack()})
        return ret
...
class Address(Anything):
    ...
    def __str__(self):
        return "[%s%s]\n%s" % (self.__class__.__name__, self.getId(), super().__str__())
...
class Route(Anything):
    ...
    def __init__(self, destinationId, originId, preferredTransport=None, transportNames=None, waypoints=[]):
        self.id = "temporary_id_source:%s_destination:%s" % (originId, destinationId)
        super().__init__(self.id, None)

        if originId is None: originId = ''
        if destinationId is None: destinationId = ''

        self.putInBackpack(self.ORIGIN, originId)
        self.putInBackpack(self.DESTINATION_KEY, destinationId)
    ...
    def getDestinationId(self):
        return self.getBackpack(self.DESTINATION_KEY)
    ...
    def __str__(self):
        return "[%s%s]\n%s" % \
               (self.__class__.__name__, self.getId(), super().__str__())
...
class VTContainer(Container):
    """
    To be used in vt-graph
    https://github.com/VirusTotal/vt-graph-api/blob/de4653b16cca12e078f597beda0b9b152dd25007/vt_graph_api/node.py#L36
    https://virustotal.github.io/vt-graph-api/quickstart.html#nodes
    """
    # vt_graph_api definitions:
    FILE = "file"
    URL = "url"
    DOMAIN = "domain"
    IP_ADDRESS = "ip_address"
    VT_TYPES = [FILE, URL, DOMAIN, IP_ADDRESS]
    DEF_TYPE = FILE
    ...
    def __init__(self, name, type):
        if type is None:
            ...
            self.type = type
        self.inVT = type in self.VT_TYPES
...
class Context:
    ...
    def __init__(self, results_path):
        ...
        self.FILE_ABSOLUTE_PATH = "%s/context/%s" % (results_path, Context.FILE_NAME) # Default file to print the context
        self.initmeplease()
    ...
    def initmeplease(self):
        ...
        self.description = 'JUDAS Context v.1.0'
    ...
    def getDefaultFilePath(self):
        """
        :return: absolute path to the file to save the context
        """
        return self.FILE_ABSOLUTE_PATH

    def setDescription(self, text):
        """
        :param text: description to be assigned to the context
        """
    ...
    def getFilesProcessed(self):
        """
        :return: path of the files processed to build the context (if any)
        """
    ...
    def showContext(self, output='context', objectsofinterest=[User.__name__, Device.__name__], mode=0):
        ...
        if mode>0:
            ...
            json.dump(d, open('auxiliar/force/force.json', 'w'))

            # Serve the file over http to allow for cross origin requests
            httpd = http_server.load_url('../auxiliar/force/judas.html')
    ...
    def __str__(self):
        ...
        for k in keys:
            lo = self.getall(k)
            for o in lo:
                str = "%s%sclass:%s\n\n" % (str,o.__str__(),o.__class__.__name__)
    ...
    def createContextFromJson(self, pathtoJsonFiles, showContext=False, printToFile=None):
        """
        ...
        """
        if printToFile is None: printToFile = self.getDefaultFilePath()
        ...
        if filepaths is not None and len(filepaths)>0:
            ...
            self.files_processed = list(set(files_processed))
            return self.files_processed
