...
import auxiliar.external.eml_parser.decode
import auxiliar.external.eml_parser.routing
    ...
    if msg.is_multipart():
        ...
        if ('content-disposition' not in msg and msg.get_content_maintype() == 'text') \
            ...
            if charset is None:
                ...
                raw_body_str = auxiliar.external.eml_parser.decode.decode_string(raw_body_str, None)
            else:
                try:
...
def prepare_multipart_part_attachment(msg: email.message.Message, counter: int = 0,
    ...
    if ('content-disposition' in lower_keys and msg.get_content_disposition() != 'inline') \
        ...
        if filename == '':
            ...
            filename = auxiliar.external.eml_parser.decode.decode_field(filename)
...
def decode_email_b(eml_file: bytes, include_raw_body: bool = False, include_attachment_data: bool = False,
    ...
    """
    if email_force_tld:
        auxiliar.external.eml_parser.regex.email_regex = auxiliar.external.eml_parser.regex.email_force_tld_regex
        auxiliar.external.eml_parser.regex.parsing_email_force_tld = True
...
def get_uri_ondata(body: str) -> typing.List[str]:
    ...
    for match in auxiliar.external.eml_parser.regex.url_regex_simple.findall(body):
...
def headeremail2list(mail: email.message.Message, header: str) -> typing.List[str]:
    ...
    try:
        ...
        field = email.utils.getaddresses(auxiliar.external.eml_parser.decode.workaround_bug_27257(mail, header))

    return_field = []

    for m in field:
        if not m[1] == '':
            if auxiliar.external.eml_parser.regex.parsing_email_force_tld:
                if auxiliar.external.eml_parser.regex.email_force_tld_regex.match(m[1]):
...
def parse_email(msg: email.message.Message, include_raw_body: bool = False, include_attachment_data: bool = False,
    ...
    headers_struc['subject'] = auxiliar.external.eml_parser.decode.decode_field(subject)
    ...
    try:
        ...
        _from = auxiliar.external.eml_parser.decode.workaround_bug_27257(msg, 'from')
    ...
    if msg_header_field != '':
        m = auxiliar.external.eml_parser.regex.email_regex.search(msg_header_field)
        if m:
    ...
    if 'date' in msg:
        try:
            headers_struc['date'] = auxiliar.external.eml_parser.decode.robust_string2date(msg.get('date'))
    ...
    try:
        ...
        for received_line in msg.get_all('received', []):
            ...
            #   with string
            #   warning list
            parsed_routing = auxiliar.external.eml_parser.routing.parserouting(received_line_flat)
            ...
            # search for domain
            for m in auxiliar.external.eml_parser.regex.recv_dom_regex.findall(received_line_flat):
                try:
            ...
            for mail_candidate in auxiliar.external.eml_parser.regex.email_regex.findall(received_line_flat):
    ...
    for body_tup in raw_body:
        ...
        if len(body) < 4096:
            ...
            for match in auxiliar.external.eml_parser.regex.email_regex.findall(body):
                list_observed_email.append(match.lower())
            for match in auxiliar.external.eml_parser.regex.dom_regex.findall(body):
                list_observed_dom.append(match.lower())
            for match in auxiliar.external.eml_parser.regex.ipv4_regex.findall(body):
                if not auxiliar.external.eml_parser.regex.priv_ip_regex.match(match):
                    if match not in pconf['whiteip']:
                        list_observed_ip.append(match)
            for match in auxiliar.external.eml_parser.regex.ipv6_regex.findall(body):
                if not auxiliar.external.eml_parser.regex.priv_ip_regex.match(match):
            ...
            for scn_pt in findall('@', body):
                ...
                for match in auxiliar.external.eml_parser.regex.email_regex.findall(body[scn_pt - 64:scn_pt + 255]):
                    list_observed_email.append(match.lower())
            ...
            for scn_pt in findall('.', body):
                ...
                for match in auxiliar.external.eml_parser.regex.dom_regex.findall(body[scn_pt - 253:scn_pt + 1004]):
                    list_observed_dom.append(match.lower())

                # Find IPv4 addresses
                for match in auxiliar.external.eml_parser.regex.ipv4_regex.findall(body[scn_pt - 11:scn_pt + 3]):
                    if not auxiliar.external.eml_parser.regex.priv_ip_regex.match(match):
                        if match not in pconf['whiteip']:
                            list_observed_ip.append(match)
            ...
            for scn_pt in findall(':', body):
                ...
                for match in auxiliar.external.eml_parser.regex.ipv6_regex.findall(body[scn_pt - 4:scn_pt + 35]):
                    if not auxiliar.external.eml_parser.regex.priv_ip_regex.match(match):
                        if match.lower() not in pconf['whiteip']:
                            list_observed_ip.append(match.lower())
    ...
    for k in set(msg.keys()):
        ...
        try:
            ...
            decoded_values = auxiliar.external.eml_parser.decode.workaround_bug_27257_field_value(msg, k)
