...
def noparenthesis(line: str) -> str:
    ...
    while not idem:
        lline = line_
        line_ = auxiliar.external.eml_parser.regex.noparenthesis_regex.sub('', line_)
        if lline == line_:
            idem = True
...
def cleanline(line: str) -> str:
    ...
    if line == '':
        ...
        return auxiliar.external.eml_parser.regex.cleanline_regex.sub('', line)
...
def give_dom_ip(line: str) -> typing.List[str]:
    ...
    """
    m = auxiliar.external.eml_parser.regex.dom_regex.findall(" " + line) + auxiliar.external.eml_parser.regex.ipv4_regex.findall(line) + auxiliar.external.eml_parser.regex.ipv6_regex.findall(line)
...
def parserouting(line: str) -> typing.Dict[str, typing.Any]:
    ...
    npline = npline.strip('\n')  # Remove any NL
    raw_find_data = auxiliar.external.eml_parser.regex.date_regex.findall(npline)  # extract date on end line.
    ...
    if npdate:
        ...
        reg += auxiliar.external.eml_parser.regex.escape_special_regex_chars.sub(r'''\\\1''', npdate)
    ...
    for item in borders:  # type: ignore
        try:
            ...
            pass
    out['date'] = auxiliar.external.eml_parser.decode.robust_string2date(npdate)
    ...
    if out.get('for'):
        ...
        m = auxiliar.external.eml_parser.regex.email_regex.findall(out['for'])
        if m:
            out['for'] = list(set(m))
        else:
            del out['for']
